...
if [ -z $TMUX ]; then
  ...
  if [ ! -f "$SYMLINK_ARRAY_PATH" ]; then
    ...
    $ROMA_UTILITIES_PATH/scripts/detacher.sh $ROMA_UTILITIES_PATH/scripts/createRosSymlinkDatabase.sh
  fi
fi
...
sshkey() {
  ...
  local git_user_name=$(grep -hr "$SSH_KEY_NAME" $ROMA_UTILITIES_PATH/dotssh/git_usernames | cut -d ' ' -f2-)
...
cbl () {
  ...
  if [ $# -eq 0 ]; then
    ...
    workspace_path=$( python $ROMA_UTILITIES_PATH/scripts/get_ros_workspace_path.py -p "$package" )
    cd "$workspace_path"
